...
class FourierUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, groups=1, spatial_scale_factor=None, spatial_scale_mode='bilinear',
                 spectral_pos_encoding=False, use_se=True, se_kwargs=None, ffc3d=False, fft_norm='ortho'):
        # bn_layer not used
...
class AttFFC(nn.Module):
    ...
    def __init__(self, ngf):
        ...
        self.add = FFC_BN_ACT(ngf, ngf, kernel_size=3, stride=1, padding=1,
                           norm_layer=nn.BatchNorm2d, activation_layer=_act_ctor,
                           **{"ratio_gin": 0.75, "ratio_gout": 0.75, "enable_lfu": False})
        self.minus = FFC_BN_ACT(ngf, ngf, kernel_size=3, stride=1, padding=1,
                           norm_layer=nn.BatchNorm2d, activation_layer=_act_ctor,
                           **{"ratio_gin": 0, "ratio_gout": 0.75, "enable_lfu": False})
        self.mask = FFC_BN_ACT(ngf, ngf, kernel_size=3, stride=1, padding=1,
    ...
    def forward(self, x):
        ...
        mask_l, mask_g = self.mask((x_l, x_g))

        add_l, add_g = self.minus(torch.cat([x_l * mask_l, x_g * mask_g], 1))

        minus_l, minus_g = self.add((x_l - add_l, x_g - add_g))

        x_l, x_g = x_l + add_l * mask_l - minus_l * (1 - mask_l), x_g + add_g * mask_g - minus_g * (1 - mask_g)
        # x_l, x_g = x_l + add_l - minus_l, x_g + add_g - minus_g
