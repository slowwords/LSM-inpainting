...
def calculate_metrics(folder1, folder2):
    ...
    # build detector
    detector_url = './pre_ckpt/inception-2015-12-05.pt'
...
def compute_fid_pids_uids(gt, comp, device):
    ...
    stat1 = FeatureStats(capture_all=True, capture_mean_cov=True, max_items=1)
    stat2 = FeatureStats(capture_all=True, capture_mean_cov=True, max_items=1)

    with torch.no_grad():
        gt = (gt + 1) * 127.5
        comp = (comp + 1) * 127.5
        fea1 = detector(gt, **detector_kwargs)
        stat1.append_torch(fea1, num_gpus=1, rank=0)
        fea2 = detector(comp, **detector_kwargs)
        stat2.append_torch(fea2, num_gpus=1, rank=0)
...
if __name__ == '__main__':
    ...
    gt = torch.from_numpy(gt).unsqueeze(dim=0).permute(0, 3, 1, 2) / 127.5 - 1
    out = torch.from_numpy(out).unsqueeze(dim=0).permute(0, 3, 1, 2) / 127.5 - 1
    fid, pids, uids = compute_fid_pids_uids(gt, out, "cpu")
